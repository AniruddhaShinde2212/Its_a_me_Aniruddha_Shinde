...
const METRICS = [
  {
    key: "pipelines_delivered",
    val: "50+",
    unit: "pipelines",
    status: "healthy",
  },
  {
    key: "data_processed_daily",
    val: "10B+",
    unit: "rows/day",
    status: "healthy",
  },
...
const ENDPOINTS = [
  {
    ...
    response: "aniruddhashinde2212@gmail.com",
    status: 200,
  },
  {
    method: "GET",
    path: "/api/contact/phone",
    response: "+91 7038182738",
    status: 200,
  },
  {
    method: "GET",
    path: "/api/social/linkedin",
    response: "linkedin.com/in/aniruddhas1",
    status: 200,
  },
  {
    method: "GET",
    path: "/api/social/github",
    response: "github.com/aniruddhashinde2212",
    status: 200,
  },
  {
    method: "GET",
    path: "/api/location",
    response: "Pune, Maharashtra, India",
    status: 200,
  },
];
            ...
            {METRICS.map((m) => (
              <div key={m.key} className="health-card">
                ...
                <div className="health-card__val">
                  {m.val}
                  <span className="health-card__unit"> {m.unit}</span>
                </div>
              ...
              <div className="api-explorer">
                {ENDPOINTS.map((ep, i) => (
                  <div
                    ...
                    onClick={() =>
                      setActiveEndpoint(activeEndpoint === i ? null : i)
                    }
                  >
                    <div className="api-endpoint__header">
                      <span className="api-method badge badge--green">
                        {ep.method}
                      </span>
                      <span className="api-path code">{ep.path}</span>
                      <span className="api-status badge badge--cyan">
                        HTTP {ep.status}
                      </span>
                    </div>
                    {activeEndpoint === i && (
                      <div className="api-response">
                        ...
                        <div>
                          ...
                          <span className="num">{ep.status}</span>,
                        </div>
                        <div>
                          {" "}
                          <span className="str">"data"</span>:{" "}
                          <span className="str">"{ep.response}"</span>
                        </div>
                        <div>{"}"}</div>
                      </div>
                    )}
              ...
              {sent ? (
                <div className="form-success">
                  <div className="form-success__code">
                    ...
                    <span className="str">CREATED</span>
                  </div>
                  <div className="form-success__body">
                    ...
                    <div>
                      {" "}
                      <span className="str">"status"</span>:{" "}
                      <span className="str">"message_queued"</span>,
                    </div>
                    <div>
                      {" "}
                      <span className="str">"eta"</span>:{" "}
                      <span className="str">"within 24 hours"</span>
                    </div>
                    <div>{"}"}</div>
                  </div>
                  <button
                ...
                <div className="contact-form">
                  <div className="cmt form-comment">
                    // Fill request body fields below
                  </div>
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      <span className="str">"subject"</span>:
                    </label>
                    <input
                      className="form-input"
                      name="subject"
